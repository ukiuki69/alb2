...
export const formatTelNum = (telnum) => {
  ...
  if (telnum.indexOf('-') > -1) return { result: true, format: telnum, num: telnum };
  ...
  const num = telnum.replace(/[\(\)\-\s]/g, '');
  if (isNaN(num)) return { result: false, format: '', num: '', err: 1 };

  const len = num.length;
  if (len < 10 || len > 11) return { result: false, format: '', num, err: 2 };
  if (num.substr(0, 1) !== '0') return { result: false, format: '', num, err: 3 };
  ...
  if (match) {
    ...
    return { result: r, format: frm, num, match };
  }

  if (ip.indexOf(num.substr(0, 3)) > -1) match = true;
  if (match) {
    frm = len === 10 ? splitByPos(num, [3, 3, 4]) : splitByPos(num, [3, 4, 4]);
    return { result: true, format: frm, num, match };
  }

  if (fd.indexOf(num.substr(0, 4)) > -1) match = true;
  if (match) {
