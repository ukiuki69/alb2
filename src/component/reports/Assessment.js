...
import { AssessmentSheet } from './Sheets/AssessmentSheet';
...
export const Assessment = (props) => {
  ...
  // 印刷設定を取得
  const useOriginalLabel = com?.ext?.reportsSetting?.usersPlan?.assessmentUseOriginalLabel ?? false;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await univApiCall(prms, 'ASS3191', '', setSnack);
        
        // API呼び出し後もコンポーネントがマウントされていることを確認
        if (isMounted && res && res?.data?.result) {
    ...
    return () => {
      isMounted = false;  // クリーンアップ時に参照を更新
    };
  }, [loadingStatus.loaded, stdDate, hid, bid]);  // allPersonalDataは依存配列から除外

  if(preview !== 'アセスメントシート') return null;
  ...
  const sheets = dataList.filter(data => {
    ...
    const prevCreated = data.created;
    const sheetProps = {user, content, created: prevCreated, useOriginalLabel};
    return(
      <AssessmentSheet key={`AssessmentSheet${i}`} {...sheetProps} />
    )
  });
