...
import { ConferenceNoteSheet } from './Sheets/ConferenceNoteSheet';
...
export const ConferenceNote = (props) => {
  ...
  if(preview !== '担当者会議議事録') return null;
  ...
  const sheets = dataList.filter(data => {
    ...
    const sheetProps = {user, content, created};
    return(
      <ConferenceNoteSheet key={`AssessmentSheet${i}`} {...sheetProps} />
    )
  });
