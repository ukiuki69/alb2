...
export const MonitoringSenmon = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const prms = {a: 'fetchUsersPlan', hid, bid, item: 'monitoringSenmon', limit};
        ...
        if (isMounted.current) {
          console.error('MonitoringSenmon データ取得エラー:', error);
        }
      }
    };
  ...
  if(!preview.includes('モニタリング') || !preview.includes('専門')) return null;
  ...
  const normalizePersonalSupportContent = (pSContent, monitoringContent) => {
    ...
    // 専門支援データのキー差分を吸収し、MonitoringSheetの期待形式にそろえる
    const normalizedGoals = supportGoals.map((goal) => {
      const source = goal ?? {};
      const needsFromArea = typeof source["五領域"] === 'string'
        ? source["五領域"].replaceAll(',', ' ／ ').replace('言語・コミュ', '言語・コミュニケーション')
        : '';
      return {
        ...source,
        "項目": source["項目"] || source["ニーズ"] || source["本人のニーズ等"] || needsFromArea || '',
        "支援目標":
          source["支援目標"] ||
          source["達成目標"] ||
          source["具体的な達成目標"] ||
  ...
  const sheets = dataList.filter(data => {
    ...
    const sheetProps = {user, content, created, pSContent, titleSuffix: '（専門）'};
    return(
      <MonitoringSheet key={`MonitoringSenmonSheet${i}`} {...sheetProps} />
    )
  });
