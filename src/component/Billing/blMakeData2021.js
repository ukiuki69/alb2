...
import {
  ...
  SOUGEY_SVC_CODE,
  KATEI_SVC_CODE,
  KESSEKI_SVC_CODE, KESSEKI_SVC_CODE2,
  IREN_SVC_CODE,
  SOUDANSIEN_SVC_CODE,
  SYOKUJI_SVC_CODE,
  SYOKAI_SVC_CODE, DADDICTION_HOHOU, KANKEIRENKEI_SVC_CODE, chiikiKubunAdult,
} from './BlCalcData2021';
...
const getKasanItem = (prms) => {
  const {
    offSchool, teiin, kubun, absence, uService, thisService, type,
    thisUser, did,
  } = prms;
  ...
  Object.keys(addiction).map(e => {
    ...
    let thisItem, hohouKasanItem = false;
...
const getBaseItemHD = (nameBase, addiction, offSchool, absence) => {
  ...
  // 平日のときに中黒つけないで検索していた！2023/10/04
...
const getBaseItemJH = (nameBase, addiction, absence) => {
  ...
  // 人欠・責任者欠がある場合、nbStrが変更になる
  if (jinketsuSekiketsu.length){
    tweekNb.forEach(e=>{
      if (nbStr === e[0]) nbStr = e[1]; 
    });
  }
  // ベース名で絞り込み
...
// ベースアイテム所得用のローダ
// 放デイと児発で処理を切り替える
const getBaseItem = (nameBase, addiction, offSchool, absence, uService, thisService) => {
  // サービスの特定
...
const getBaseItemHohouAsKasan = (
    nameBase, addiction, offSchool, absence, uService, thisService, comAdicHH
) => {
  let service;
  if (svcCnt === 1) service = uService;
  else if (thisService) service = thisService;
...
const getKasanOneItemJH = (key, addiction, teiin, type) => {
  ...
  // 看護加配は児童タイプと施設タイプが重身になっていないと空白のデータオブジェクトを返す
  if (kangoKahaiIsNull(addiction, key, type)) return {};

  if (key === '家庭連携加算'){
    // valueに分数が入っているのでそれでフィルタ
    const t = jihatsuKasan.filter(e=>(
  ...
  // 重身型の場合未就学児に対する区分はない。無視する 2022/07/14
  if (addiction.就学区分 === '1' && type === '障害児'){
    kasan = kasan.filter(e=>{
      const opts = e.opt.split(',');
  ...
  if (key !== '看護職員加配加算'){
    if (type === '障害児'){
      kasan = kasan.filter(e=>!e.opt.includes('重症心身障害児') && !e.opt.includes('難聴児'))
    }
    else{
      kasan = kasan.filter(e=>e.opt.includes(type));
    }
  }
  if (kasan.length === 1) return kasan[0];
  if (!kasan.length) return errObj;
  ...
  // 定員による絞り込み
  kasan = kasan.filter(e=>parseInt(e.teiin)>=parseInt(teiin));
  if (kasan.length === 1) return kasan[0];
  if (!kasan.length) return errObj;
  // 定員順にソート
...
const getKasanOneItemHH = (key, addiction, type) => {
  ...
  let kasan = hohouKasan.filter(e => (e.name === key && e.value === value));
  // 延長支援加算 検出
  kasan = getEntyouKasan(kasan, key, type);
  // これだけで特定できたらそのままリターン
  if (kasan.length === 1) return kasan[0];
...
const getKasanOneItemSoudan = (key, addiction, service) => {
  ...
  // カウントプロパティーを持つ加算オブジェクトがあるので、カウントに値を入れておく
  if (!isNaN(value)){
    // tLst.filter(e=>('count' in e)).forEach((e, i)=>{
    //   tLst[i].count = parseInt(value);
    // })
    tLst = tLst.map(e => {
      if ('count' in e) {
        return { ...e, count: parseInt(value) };
      }
      return e;
    });
  }
...
const kangoKahaiIsNull = (addiction, key, type) => {
  const shisetsuType = parseInt(addiction.重症心身型);
  if (key !== '看護職員加配加算') return false;
  if (shisetsuType && type !== '重症心身障害児') return true;
  else return false;
}
...
const getKasanOneItem = (key, addiction, offSchool, teiin, type, kubun) => {
  ...
  // 欠席時対応加算の例外処理
  const kesseJuushin = type === '重症心身障害児' && shisetsuType === 1;
  if (key === '欠席時対応加算'){
    let kesseki;
    if (kesseJuushin){
  ...
  kasan = kasan.filter(e => {
    if (key === '看護職員加配加算'){
      return (
        e.opt.includes(teiinStr) && 
        e.opt.includes(schoolOffStr)
      )
    }
...
// 手動の場合は無条件付与
// 旧版 2022-07以前に適用
const judgeJougenkanriKasanOld = (users, uid, schedule) => {
  ...
  if (uSch === undefined) return false;
  if (Array.isArray(uSch)) return false;
  let cnt = 0;
...
// 上限管理加算を判断して付与するべきなら上限管理加算アイテムを返す
// 手動の場合は無条件付与
const judgeJougenkanriKasan = (users, uid, schedule, schTmp) => {
  const fdp = (o, k) => comMod.findDeepPath(o, k);
  const thisUser = comMod.getUser(uid, users);
  ...
  // 兄弟上限のカウントがあり長兄であれば加算設定対象とする
  // if (cnt && isKj) return JOUGEN_KANRI[service];
  // 自分以外の兄弟のuid配列を得る
...
const totalizeItems = (tmpSch, masterRec, users, schedule, stdDate, classroom = '')=>{
  ...
  const kessekiSvc = [
    ...KESSEKI_SVC_CODE, ...SOUDANSIEN_SVC_CODE, 
    ...KATEI_SVC_CODE, ...KANKEIRENKEI_SVC_CODE,
  ];
  // 処遇と特別のサービスコード
  ...
  Object.keys(tmpSch).map(e=>{
    ...
    if (stdDate >= kyoudaiAutoTyousei){
      jougen = judgeJougenkanriKasan(users, e, schedule, tmpSch);
    }
    else{
      jougen = judgeJougenkanriKasanOld(users, e, schedule, tmpSch);
    }
    if (jougen){
  ...
  Object.keys(tmpSch).map(e=>{
    ...
    itemTotal.map(f => {
      if (f.syoguu) {
        ...
        .filter(g=>g.s && g.s.indexOf(syubetu) === 0 && !g.syoguu)
        .reduce((v, g) => (v + g.tanniNum), 0);
...
const totlizeCityAndService = (srcDt, masterRec, userlist) =>{
  ...
  cityService.sort((a, b)=>((a < b)? -1: -1)).map(e=>{
    ...
    totalized.push({
      scityNo, serviceSyubetu: sSyubetu, adjust1, adjust2, countOfUse,
...
// schTmp を受けて兄弟調整を行う。
const makeBrosTyousei = (dt, jino, users, schedule) => {
  // ------ 兄弟調整を行う
  // 長男長女のキー
  const firstUsers = Object.keys(dt)
  .filter(e=>parseInt(dt[e].brosIndex) === 1);
...
const makeJichitaiJosei = (schTmp, com, users) => {
  ...
  Object.keys(schTmp).map(e=>{
    ...
    // 独自上限のときの自治体助成金538
    else if (city.dokujiJougen && uDokujiJougen){
    ...
    if (city.dokujiJougen && schTmp[e].管理事業所){
      ...
      if (dokujiHojo){
        o.jichiJosei = Number(dokujiHojo);
        o.joseiNo = city.joseiNo;
      }
    }
  });
...
// 自自体助成金による自治体助成番号の調整を行う。助成金がある場合は記述を行い
// 助成金がない場合は記述を外す
...
const isMusyouka = (schTmp) => {
  ...
  Object.keys(schTmp).filter(e=>/^UID[0-9]+/.test(e)).map(e=>{
    ...
    const isTargetAge = uObj.ageStr.match(/^[3-5]/);
...
// 請求の基準になるschtmpを作成する
const makeSchTmp = (users, schedule,) => {
  ...
  uids.map(e => {
    ...
    if (Object.keys(u).length){
      ...
      schTmp[e].upperlimit = parseInt(u.priceLimit);
...
const makeDAddiction = (prms) => {
  ...
  Object.keys(schTmp).map(e => {
    ...
    Object.keys(schTmp[e]).map(f => {
      ...
      // この処理は別ルーチンにしたほうが良さそう。
      // 加算としての保訪がありユーザーとして保訪のサービスがありこのサービスは保訪以外
      // const hhSvcAsKasan = userHasHohou && schTmp[e][f].dAddiction 
      // && '保育訪問' in schTmp[e][f].dAddiction && thisService !== HOHOU;
      // // フラグ立てて保訪の事業所加算を取得しておく
      // schTmp[e][f].hhSvcAsKasan = hhSvcAsKasan;
      // schTmp[e][f].comAdicHH = comAdicHH;
...
const idetifyServicecode = (prms) => {
  ...
  Object.keys(schTmp).map(e => {
    ...
    if (inService(uService, HOUDAY)){
      ...
      nameBase = getServiceNameBaseJH(comAdicJH, schedule, users, e, type);
    }
    else nameBase = '';
    // 医療的ケア児には別途namebaseが必要
    let nameBaseIcare = '';
    let icareType = thisUser.icareType;
    if (inService(uService, HOUDAY) && icareType){
      nameBaseIcare = getServiceNameBase(comAdic, schedule, users, e, type, true);
    }
    else if (inService(uService, JIHATSU) && icareType){
      nameBaseIcare = getServiceNameBaseJH(comAdicJH, schedule, users, e, type, true);
    }
    // console.log(nameBase, 'nameBase')
    if (nameBase === false){
      const errUser = comMod.getUser(e, users);
      // console.log(errUser, errUser.name, 'errUser')
      const errDetail = {
        name: errUser,
        description: 
      ...
      setBillingResult.result = false;
      setBillingResult.errDetail = errDetail;
      return false;
    }
    Object.keys(schTmp[e]).map(f => {
      // didでない要素はスキップz 
      ...
      const thisNameBase = (o.dAddiction.医療ケア児基本報酬区分)? nameBaseIcare: nameBase;
      // 基本項目の追加
      const baseItem = getBaseItem(
        thisNameBase, o.dAddiction, o.offSchool, o.absence, uService, o.service
      );
      ...
      if (baseItem){
        ...
        if (schTmp[e].countOfUseMulti[o.service]){
          schTmp[e].countOfUseMulti[o.service]++
        }
        else{
          schTmp[e].countOfUseMulti[o.service] = 1;
        }
      }
      ...
      // const kasanItem = (() => {
      //   if (!soudanServiceName.includes(uService)){
      //     return getKasanItem(getKasanItemPrms);
      //   }
      //   else{
      //     return getKasanItemSoudan(getKasanItemPrms);
      //   }
      // })();
      // 欠席に対してもカウントを実施。
      ...
      let transfer;
      if (inService(uService, HOUDAY)){
        transfer = getTrasferItem(o.transfer, o.absence, type, o.dAddiction,);
      }
      else if (inService(uService, JIHATSU)){
        transfer = getTrasferItemJH(o, e, users, o.dAddiction,);
      }
      if (transfer){
        items.push(...transfer);
      }
...
export const setBillInfoToSch = (prms, userlist = []) => {
  ...
  // 児発の項目も追加
  const comAdic = (hasHouday)? com.addiction.放課後等デイサービス: null;
  const ku = hasHouday? comAdic.サービス提供時間区分: '';
  // const teiin = parseInt(comAdic.定員);
  const comAdicJH = (hasJihatsu)? com.addiction.児童発達支援: null;
  const comAdicHH = (hasHohou)? com.addiction.保育所等訪問支援: null;
  const comAdicKS = (hasKeikakuSoudan)? com.addiction[KEIKAKU_SOUDAN]: null;
  const comAdicSS = (hasSyougaiSoudan)? com.addiction[SYOUGAI_SOUDAN]: null;
  ...
  // 兄弟調整を行う
  makeBrosTyousei(schTmp, com.jino, users, schedule, serviceItems);
  // 上限管理を作成する！！！
...
const billingCsvK122_4 = (outputRec, masterRec, thisDt) => {
  ...
  serviceSyubetuCd.forEach((e, i)=>{
    ...
    if (isMultiSvc){
      countOfUse = comMod.null2Zero(thisDt.countOfUseMulti[svcName])
...
const billingCsvK122_2 = (outputRec, masterRec, dt, thisUser) => {
  ...
  ary.forEach(e => {
    ...
    const countOfUse = isMltSvc? dt.countOfUseMulti[svc]: dt.countOfUse; 
    erl(outputRec, THIS_MONTH, masterRec.thisMonth);
...
const billingCsvK122_1 = (outputRec, masterRec, dt, kdj) =>{
  ...
  if (mltSvc){
    ...
    Object.keys(dt.userSanteiTotalSvc).forEach(e=>{
      ...
      if (dt.tashikeigen === 2 && Math.floor(w * 0.05) > dt.upperlimit)  v += dt.upperlimit; 
      else if (dt.tashikeigen === 2 ) v += Math.floor(w * 0.05);
      else if (dt.tashikeigen === 3) v += 0;
      else if (Math.floor(w * 0.1) > dt.upperlimit) v += dt.upperlimit;
  ...
  // 調整後利用者負担額 上限額を超えるかつ複数サービスの場合に設定
  // 複数サービスの場合このパラメータを設定
  // 2023/03/19 無償化の場合は0を入れる
...
const billingCsvUsers = (
  ...
  const target = billingDt.filter(f => (f.scityNo === scityNo));
  // 対象となる各ユーザーを処理
  target.map(f=>{
...
// 相談支援対応のためbillingCsvScityとbillingCsvScitySoudanの条件分岐を行う。
...
const C_SYOKUJITEIKYOU = 32; //食事提供加算
...
const C_SANTEINISSUU = 45; // 算定日数
const C_CNT_SYOKAI = 113; // 初回加算
...
const C_CNT_SYOKUJI = 45;//食事支援回数:
const C_GOUKEISANTEI = 37; // 合計算定回数
const C_SYOKAI = 70;
...
const makeTeikyouJissekiOneUser = (masterRec, bDt, mltSvc ,svcSyubetu, outputRec) =>{
  ...
  bDt.itemTotal.filter(e => KATEI_SVC_CODE.indexOf(e.s) > -1)
  .filter(e => e.s.indexOf(svcSyubetu + '') === 0) // 保訪対応追加。サービスコードをサービス種別で絞る
  .map(e => {
    kateiRenCnt += e.count;
    // 算定は制限回数を超えない範囲で
  ...
  let houmonCnt = 0;
  let houmonSan = 0;
  // 2021で廃止になっているのでここは出力しない
  // bDt.itemTotal.filter(e => HOUMON_SVC_CODE.indexOf(e.s) > -1)
  // .map(e => {
  //   houmonCnt += e.count;
  //   // 制限回数チェックして設定
  ...
  if (svcSyubetu !== 64){
    ...
    bDt.itemTotal.filter(e => IREN_SVC_CODE.indexOf(e.s) > -1)
  ...
  if (svcSyubetu !== 64){
    let soudanCnt = 0;
    bDt.itemTotal.filter(e => SOUDANSIEN_SVC_CODE.indexOf(e.s) > -1)
    .map(e => { soudanCnt += e.count });
    er(C_SOUDAN, soudanCnt);         //事業所内相談支援回数
  }  
  
  // 食事提供
  // console.log('食事配列', bDt.itemTotal.filter(e=>SYOKUJI_SVC_CODE.indexOf(e.s) > -1))
  const syokujiCnt = bDt.itemTotal
  .filter(e=>SYOKUJI_SVC_CODE.indexOf(e.s) > -1)
  .reduce((v, e) => (v + e.count), 0);
  er(C_CNT_SYOKUJI, syokujiCnt? syokujiCnt: '');
  
  // 合計算定回数 保訪のみ
  if (svcSyubetu === 64){
    er(C_GOUKEISANTEI, bDt.countOfUseMulti[svcName])
  }
  // 初回算定回数 保訪のみ
  if (svcSyubetu === 64){
    const v = bDt.itemTotal.filter(e => SYOKAI_SVC_CODE.includes(e.s))
    .reduce((v, e)=>(v + e.count), 0);
    er(C_CNT_SYOKAI, v);
  }
  ...
  maisaiK611_2[C_CITY] = bDt.scityNo;
  ...
  daysUsed.map(e=>{
    ...
    // 欠席時対応加算２もチェック
    const kessekiKasan2 = items.find(e=>KESSEKI_SVC_CODE2.indexOf(e.s) > -1);
    // 家庭連携加算あるか
    const karenKasan = items.find(e=>KATEI_SVC_CODE.indexOf(e.s) > -1);
    // 相談支援加算あるか
    const soudanKasan = items.find(e=>SOUDANSIEN_SVC_CODE.indexOf(e.s) > -1);
    // 医療連携あるか
    const iryouRenkei = items.find(e=>IREN_SVC_CODE.indexOf(e.s) > -1);
    // 初回加算あるか
    ...
    if (svcSyubetu !== 64){
      er(C_TEIKYOUJOUKYOU, (kessekiKasan)? 8 : '');
    }
    // // 保訪は常に1 -> 常に出力しない
    // else{
    //   er(C_TEIKYOUJOUKYOU, 1);
    // }
    er(C_DATE, parseInt(e.substring(7, 9)));// 日付の日にちだけ キーの下二桁

    // 家庭連携加算実績記述用に分数を取得 明細書き込み
    if (karenKasan){
      const min = parseInt(bDt[e].dAddiction.家庭連携加算);
      // 90分を 0150 に変換
    ...
    let teikyoukeitai = '';
    // 児発は無しで良いんだっけ？
    // if (bDt.service === '放課後等デイサービス'){ // サービス名で判断するのをやめ
    if (svcSyubetu === 63){
      teikyoukeitai = o.offSchool? 2 : 1;
    }
    // 家庭連携加算のときは空白になるらしい
    // 空白にならない！ 国保連確認済み 2024/02/10
    // if (karenKasan){
    //   teikyoukeitai = '';
    // }
...
const makeCsvFromDatas = () => {
  const arrayToCSV = data => {
    ...
    // 各要素をCSV形式の行に変換
    data.forEach(item => {
      const row = keys.map(key => item.hasOwnProperty(key) ? `"${item[key]}"` : 'undefined').join(',');
      csvRows.push(row);
    });
  ...
  const csvContent = arrayToCSV(houdySirvice);
