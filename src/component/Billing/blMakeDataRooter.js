...
import { 
  HOHOU, HOUDAY, JIHATSU, KEIKAKU_SOUDAN, SYOUGAI_SOUDAN, inService 
...
// makeJugenkanri  as makeJugenkanri2024 ,
export const makeJugenkanri = (billingDt, masterRec, jougenKubun, users, schedule) => {
  const stdDate = billingDt[0].stdDate;
  const rt = (() => {
    if (stdDate ?? '' >= '2024-04-01'){
      return makeJugenkanri2024(billingDt, masterRec, jougenKubun, users, schedule, stdDate);
    }
    else return makeJugenkanri2021(billingDt, masterRec, jougenKubun, users, schedule);
  })();
  return rt;
}
