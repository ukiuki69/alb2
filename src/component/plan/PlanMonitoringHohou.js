...
import { blue, red, teal, grey, indigo, purple } from '@material-ui/core/colors';
...
const PlanMonitoringHohouDetail = (props) => {
  const classes = useStyles();
  ...
  const isAddMode = useMemo(() => {
    return isPlanAddMode(location, 'monitoringHohou');
  }, [location.search]);
  ...
  // 実施日の変更を監視し、PersonalSupportHohouデータを更新する
  useEffect(() => {
  ...
  const handleDelete = async () => {
    if (!deleteConfirm){
      ...
      const prms = {
        a: 'deleteUsersPlan',hid, bid, uid: effectiveUid, item: 'monitoringHohou', 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res && res.data && res.data.result) {
        ...
        setSnack({ msg: 'モニタリング（保訪）が保存されました', severity: 'success' });
  ...
  return (
    ...
    >
      <PlanRelatedItemsPanel 
        ...
        inputs={inputs}
        currentPlanType="monitoringHohou"
      />
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 16 }}>
            <Button
              ...
              onClick={() => {
                ...
                setLS(PLAN_ADD_MODE_ITEM, 'monitoringHohou');
                setInputs(initialValues);
                setOriginInputs(initialValues);
                setDateDisabled(false);
              }}
...
export const PlanMonitoringHohou = () => {
  const allState = useSelector(s=>s);
  ...
  useEffect(() => {
    ...
    const fetchData = async (personalSupportOnly = false) => {
      try {
        const prmsPs = {a: 'fetchUsersPlan', hid, bid, item: 'personalSupportHohou', lastmonth: lastMonth};
        ...
        if (personalSupportOnly) {
          ...
          const prmsMo = {a: 'fetchUsersPlan', hid, bid, item: 'monitoringHohou', lastmonth: lastMonth};
        ...
        if (isMountedRef.current && resDone) {
          ...
          const errorMsg = personalSupportOnly 
            ? 'personalSupportHohouデータ取得に失敗しました' 
            : 'データ取得に失敗しました';
          setSnack({
